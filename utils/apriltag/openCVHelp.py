...
class OpenCVHelp:
    ...
    @staticmethod
    def pointsToTagCorners(points: np.ndarray) -> list[TagCorner]:
        corners = [TagCorner(p[0, 0], p[0, 1]) for p in points]
        return corners

    @staticmethod
    def cornersToPoints(corners: list[TagCorner]) -> np.ndarray:
